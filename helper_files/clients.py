...
class R2Client:
    ...
    def post_collection(self, body):
        return self._parse_response(self._session.post(self._api_url + "/chart-collections", json=body))
    ...
    def update_chart(self, old_chart_id: str, new_chart):
        ...
        if len(bars) >= 20000:
            ...
            for i in range(upload_windows):
                ...
                response = self._session.patch(self._api_url + f"/charts/{old_chart_id}", json={"bars": upload_bars})
            return response
        else:
