...
def plot_equity(i, balance, equity, data, xmin, xmax):
    if len(balance) > 0:
        balance = np.pad(balance, (xmin, len(data)-xmax-1), 'constant', constant_values = (1, balance[-1]))
        equity = np.pad(equity, (xmin, len(data)-xmax-1), 'constant', constant_values = (1, equity[-1]))
    else:
...
def plot_patterns(data, patterns, start, end, backtesting_parameters, plotting_parameters, chart, models, strategy_parameters, balance, equity):
    dates = pd.DatetimeIndex(data[get_index(start, data) : get_index(end, data)+1]["datetime"])
    ...
    start_date = dates[0]            
    xmin = get_index(start_date, data)
    xmax = get_index(dates[-1], data)
    ymin = min(data["low"][xmin:xmax])
    ymax = max(data["high"][xmin:xmax])
    height = ymax - ymin
    mc = mpf.make_marketcolors(up="limegreen", down="red", inherit=True)
    s = mpf.make_mpf_style(base_mpf_style="nightclouds", marketcolors=mc)
    fig, axs = mpf.plot(df, type="candle", style=s, returnfig=True,addplot=plots + plot_equity(i, balance, equity, data, xmin, xmax), warn_too_much_data=len(df) + 1)    
    if plotting_parameters["show_patterns"]:
        for pattern in patterns:
            if type(pattern) != int:
                axs[0], rectangle, in_dates = plot_rectangle(axs[0], data, pattern, dates)
                axs[0] = (
                    plot_trendlines(axs[0], data, pattern)
                    if in_dates and plotting_parameters["show_trendlines"]
                    else axs[0]
                )
    axs[0].set_xlim(xmin=xmin, xmax=xmax)
...
def plot_rectangle(ax, data, pattern, dates):
    ...
    for shape in pattern["shapes"]:
        if shape["shape_type"] == "rectangle":
            rectangle = pd.DataFrame(shape["points"])
            rectangle["datetime"] = pd.to_datetime(rectangle["datetime"])
            if rectangle["datetime"][1] in dates:
                width = get_index(rectangle["datetime"][1], data) - get_index(rectangle["datetime"][0], data)
                height = rectangle["price"][1] - rectangle["price"][0]
                rect = Rectangle(
                    (get_index(rectangle["datetime"][0], data), rectangle["price"][0]),
                    width,
                    height,
                    linestyle="dashed",
                    linewidth=1.5,
                ...
                ax.add_patch(rect)
                ax.text(
                    (get_index(rectangle["datetime"][0], data) + (0.5 * width)),
                    (rectangle["price"][0] + (1.025 * height)),
                ...
                in_dates = True
    return ax, rectangle, in_dates


def plot_trendlines(ax, data, pattern):
    for shape in pattern["shapes"]:
        if shape["shape_type"] == "trend_line":
            line = pd.DataFrame(shape["points"])
            line["datetime"] = pd.to_datetime(line["datetime"])
            ax.plot(
                [get_index(line["datetime"][0], data), get_index(line["datetime"][1], data)],
                [line["price"][0], line["price"][1]],
                color="white",
                linewidth=1.5,
            )
            if len(line) > 3:
                ax.plot(
                    [get_index(line["datetime"][2], data), get_index(line["datetime"][3], data)],
                    [line["price"][2], line["price"][3]],
                    color="white",
                    linewidth=1.5,
                )
    return ax
